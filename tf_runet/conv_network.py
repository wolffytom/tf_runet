...
class Conv_Net(BasicACNetwork):
    def __init__(self, name, nx, ny, channels, n_class, filter_size=3, cost="cross_entropy", cost_kwargs={}, use_mark = False):
        ...
        with tf.variable_scope('RU_Net', reuse = tf.AUTO_REUSE):
            ...
            self.total_accuracy, self.class_accuracy = self._get_accuracy(self.predict, self.cutlabels)
    ...
    def _block_rnn(self, in_node, batch_size, steps, sx, sy, in_channels, out_channels, initstate, LSTM = True):
        ...
        if LSTM is True:
            in_node, variables = block_c_lstmnn(sx, sy, in_node, in_channels, out_channels, initstate)
        else:
    ...
    def _create_ru_net_sp_init(self, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True, LSTM = True):
    ...
    def _create_ru_net_zero_init(self, 
            layers=3, 
            features_root=16, 
            filter_size=3, 
            pool_size=2, 
